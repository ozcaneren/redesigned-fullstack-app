...
export default function HeaderAddTitleModal() {
  ...
  const [headerTitle_en, setHeaderTitle_en] = useState("");

  const handleSubmit = async () => {
    try {
      const newHeader = {
        headerTitle: headerTitle,
        headerTitle_en: headerTitle_en,
      };
      await axios.post("http://localhost:4000/api/header/texts", newHeader);
      setHeaderTitle("");
      setHeaderTitle_en("");
    } catch (error) {
      console.error(error);
                  ...
                  <div className="relative p-6 flex-auto">
                    ...
                    <div className="mb-4">
                      ...
                      <input
                        className="shadow appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline"
                        type="text"
                        value={headerTitle_en}
                        onChange={(e) => setHeaderTitle_en(e.target.value)}
                      />
                    </div>
                  </div>
