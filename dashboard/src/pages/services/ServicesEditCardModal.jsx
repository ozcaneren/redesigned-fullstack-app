...
const ServicesEditCardModal = ({
  servicesId,
  showCardModal,
  setShowCardModal,
}) => {
  ServicesEditCardModal.propTypes = {
  ...
  const fetchServicesById = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:4000/api/service/cards/${servicesId}`
      );
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:4000/api/service/cards/${servicesId}`, {
        ...services,
      });
...
export default ServicesEditCardModal;
