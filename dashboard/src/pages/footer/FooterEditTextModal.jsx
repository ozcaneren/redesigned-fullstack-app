...
const FooterEditTextModal = ({ footerId, showTextModal, setShowTextModal }) => {
  FooterEditTextModal.propTypes = {
    footerId: PropTypes.string,
    showTextModal: PropTypes.bool,
    setShowTextModal: PropTypes.func,
  };
  ...
  const fetchFooterById = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:4000/api/footer/text/${footerId}`
      );
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:4000/api/footer/text/${footerId}`, {
        ...footer,
      });
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                <div className="flex items-start justify-between p-5 border-b border-solid border-gray-300 rounded-t">
                  <h3 className="text-2xl text-modalMainText font-semibold">
                    Footer Baslik ve Metin Duzenle
                  </h3>
                  <button
...
export default FooterEditTextModal;
