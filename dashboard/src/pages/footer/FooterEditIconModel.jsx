...
const FooterEditIconModel = ({ footerId, showIconModal, setShowIconModal }) => {
  FooterEditIconModel.propTypes = {
    footerId: PropTypes.string,
    showIconModal: PropTypes.bool,
    setShowIconModal: PropTypes.func,
  };
  ...
  const fetchFooterById = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:4000/api/footer/icon/${footerId}`
      );
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:4000/api/footer/icon/${footerId}`, {
        ...footer,
      });
...
export default FooterEditIconModel;
